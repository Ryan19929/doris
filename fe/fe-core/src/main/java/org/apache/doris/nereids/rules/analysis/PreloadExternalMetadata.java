 ...
 */
public class PreloadExternalMetadata {
    ...
    private boolean preloadExternalTable(StatementContext statementContext, ExternalTablePreloadInfo preloadInfo) {
        ...
        if (!table.supportsExternalMetadataPreload()) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("{} skip external metadata preload for unsupported table {}",
                        getPreloadQueryIdentifier(statementContext), getExternalTableLogName(table));
            }
            return false;
        }
