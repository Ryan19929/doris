...
public class CatalogRecycleBinTest {
    ...
    private static final long IMAGE_TEST_DB_ID = 10001;
    private static String runningDir;
    private static Env env;
    private static int originalMaxSameNameCatalogTrashNum;

    @BeforeClass
    public static void beforeClass() throws Exception {
        FeConstants.runningUnitTest = true;
        originalMaxSameNameCatalogTrashNum = Config.max_same_name_catalog_trash_num;
    ...
    public void afterTest() {
        Config.max_same_name_catalog_trash_num = originalMaxSameNameCatalogTrashNum;
    }
    ...
    public void testRecoverEmptyDatabase() throws Exception {
        ...
        Assert.assertFalse(recycleBin.isRecycleDatabase(CatalogTestUtil.testDbId1));
        Assert.assertFalse(recycleBin.dbNameToIds.containsKey(CatalogTestUtil.testDb1));
    }

    @Test
    public void testImageRoundTripRebuildsNameCaches() throws Exception {
        CatalogRecycleBin recycleBin = Env.getCurrentRecycleBin();

        Database emptyDb = new Database(CatalogTestUtil.testDbId1, CatalogTestUtil.testDb1);
        Assert.assertTrue(recycleBin.recycleDatabase(emptyDb, Sets.newHashSet(), Sets.newHashSet(),
                false, false, 0));
        ...
        Assert.assertTrue(recycleBin.recycleTable(IMAGE_TEST_DB_ID, olapTable, false, false, 0));

        Partition partition = olapTable.getPartition(CatalogTestUtil.testPartitionId1);
        Assert.assertTrue(recycleBin.recyclePartition(
                IMAGE_TEST_DB_ID,
                CatalogTestUtil.testTableId1,
                CatalogTestUtil.testTable1,
                partition,
        ...
        MetaContext originalMetaContext = MetaContext.get();
        MetaContext currentMetaContext = new MetaContext();
        currentMetaContext.setMetaVersion(FeMetaVersion.VERSION_CURRENT);
        currentMetaContext.setThreadLocalInfo();
        CatalogRecycleBin restored;
        try {
            ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
            recycleBin.write(new DataOutputStream(byteStream));
            restored = CatalogRecycleBin.read(
                    new DataInputStream(new ByteArrayInputStream(byteStream.toByteArray())));
        } finally {
            if (originalMetaContext == null) {
                MetaContext.remove();
            } else {
                originalMetaContext.setThreadLocalInfo();
            }
        }

        assertNameCaches(restored);

        restored.dbNameToIds.clear();
        restored.dbIdTableNameToIds.clear();
        restored.dbTblIdPartitionNameToIds.clear();
        restored.gsonPostProcess();
        assertNameCaches(restored);
    }

    private void assertNameCaches(CatalogRecycleBin recycleBin) {
        Assert.assertEquals(Sets.newHashSet(CatalogTestUtil.testDbId1),
                recycleBin.dbNameToIds.get(CatalogTestUtil.testDb1));
        Assert.assertEquals(Sets.newHashSet(CatalogTestUtil.testTableId1),
                recycleBin.dbIdTableNameToIds.get(Pair.of(IMAGE_TEST_DB_ID, CatalogTestUtil.testTable1)));
        Assert.assertEquals(Sets.newHashSet(CatalogTestUtil.testPartitionId1),
                recycleBin.dbTblIdPartitionNameToIds
                        .get(Pair.of(IMAGE_TEST_DB_ID, CatalogTestUtil.testTableId1))
                        .get(CatalogTestUtil.testPartition1));
    }
    ...
    @Test
    public void testReplayRecoverPartitionWithNewNameClearsNameCache() throws Exception {
        CatalogRecycleBin recycleBin = Env.getCurrentRecycleBin();
        Database db = CatalogTestUtil.createSimpleDb(
                CatalogTestUtil.testDbId1,
                CatalogTestUtil.testTableId1,
        ...
        String newPartitionName = "recoveredPartition";
        recycleBin.replayRecoverPartition(olapTable, CatalogTestUtil.testPartitionId1, newPartitionName);

        Assert.assertFalse(recycleBin.isRecyclePartition(
                CatalogTestUtil.testDbId1, CatalogTestUtil.testTableId1, CatalogTestUtil.testPartitionId1));
        Assert.assertFalse(recycleBin.dbTblIdPartitionNameToIds.containsKey(
                Pair.of(CatalogTestUtil.testDbId1, CatalogTestUtil.testTableId1)));
        Assert.assertNotNull(olapTable.getPartition(newPartitionName));
    }
    ...
    public void testExpiredDatabaseSnapshotDoesNotEraseNewGeneration() throws Exception {
        ...
        Assert.assertTrue(recycleBin.recycleDatabase(database, Sets.newHashSet(), Sets.newHashSet(),
                false, false, 0));
        ...
        runEraseAfterReplacingGeneration(recycleBin, "eraseDatabase", () -> {
            ...
            Assert.assertTrue(recycleBin.recycleDatabase(recovered, Sets.newHashSet(), Sets.newHashSet(),
                    false, false, 0));
        });

        Assert.assertTrue(recycleBin.isRecycleDatabase(database.getId()));
        Assert.assertTrue(recycleBin.getRecycleTimeById(database.getId()) > 0);
    }
    ...
    public void testExpiredTableSnapshotDoesNotEraseNewGeneration() throws Exception {
        ...
        Assert.assertTrue(recycleBin.recycleTable(database.getId(), table, false, false, 0));
        recycleBin.setRecycleTimeByIdForReplay(table.getId(), 0L);

        runEraseAfterReplacingGeneration(recycleBin, "eraseTable", () -> {
            Assert.assertTrue(recycleBin.recoverTable(database, table.getName(), table.getId(), null));
            Assert.assertTrue(recycleBin.recycleTable(database.getId(), table, false, false, 0));
        });

        Assert.assertTrue(recycleBin.isRecycleTable(database.getId(), table.getId()));
        Assert.assertTrue(recycleBin.getRecycleTimeById(table.getId()) > 0);
    }
    ...
    public void testExpiredPartitionSnapshotDoesNotEraseNewGeneration() throws Exception {
        ...
        Assert.assertTrue(recycleBin.isRecyclePartition(database.getId(), table.getId(), partition.getId()));
        Assert.assertTrue(recycleBin.getRecycleTimeById(partition.getId()) > 0);
    }
    ...
    private void recyclePartition(CatalogRecycleBin recycleBin, Database database,
        ...
        Assert.assertTrue(recycleBin.recyclePartition(database.getId(), table.getId(), table.getName(),
    ...
    private void runEraseAfterReplacingGeneration(CatalogRecycleBin recycleBin,
        ...
        try {
            ...
            Assert.assertTrue("Expired snapshot was not collected",
                    snapshotCollected.await(10, TimeUnit.SECONDS));
            ...
            executor.shutdown();
            try {
                Assert.assertTrue("Erase worker did not finish",
