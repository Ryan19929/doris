...
public class Config extends ConfigBase {
    ...
    @ConfField(mutable = true, description = {
            "当表只有两个副本时，是否忽略 min_load_replica_num=1 并强制要求两个副本均写入成功。"
                    + "动态修改时需使用 ADMIN SET ALL FRONTENDS CONFIG，使所有 FE 保持一致。",
            "Whether to ignore min_load_replica_num=1 and require both replicas to be written successfully "
                    + "for tables with two replicas. Use ADMIN SET ALL FRONTENDS CONFIG for dynamic changes "
                    + "so that all FEs use the same value."})
